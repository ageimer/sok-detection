...
module Solver () : Solver_sig.S = struct
  ...
  let assignment x = Term.Bv.assignment x
   ...
   *   | Sat ->
   *       Checkct_options.Logger.fatal "(%a %a %a) <> %a" T.pp_op f T.pp x T.pp y T.pp
   *         e
   *   | _ -> close ()
   *
   * let visit_unop =
   ...
   *   match check_sat () with
   *   | Sat -> Checkct_options.Logger.fatal "(%a %a) <> %a" T.pp_op f T.pp x T.pp e
   *   | _ -> close () *)
end

...
module Insec_model = struct
  type t = {
    public : Bitvector.t list S.t;
    secret1 : Bitvector.t list S.t;
    secret2 : Bitvector.t list S.t;
  }
      ...
      S.iter (
        fun name list -> 
          Format.fprintf ppf "@[<h>  %s : @[<hov>%a@]@]@," name
            (Format.pp_print_list ~pp_sep:Format.pp_print_space Bitvector.pp_hex_or_bin)
            list;
      ) vars 
    in
    Format.fprintf ppf "@[<v 0>@[<v 1>public :@,%a@]@,@[<v 1>secret1 :@,%a@]@,@[<v 1>secret2 :@,%a@]@]"
      pp_category t.public pp_category t.secret1 pp_category t.secret2

end

module Ct_status = struct
  type t = CT_Secure | CT_Insecure of Insec_model.t | CT_Unknown
  ...
  let pp ppf = function
  | CT_Secure -> Format.pp_print_string ppf "secure"
  | CT_Insecure _ -> Format.pp_print_string ppf "insecure"
  | CT_Unknown -> Format.pp_print_string ppf "unknown"
end
...
module type STATE = sig
  ...
  val fresh : string -> int -> ?secret:bool -> t -> t
  ...
  val ctstatus_and : (t -> Ct_status.t * t) -> (t -> Ct_status.t * t) -> t -> Ct_status.t * t

  val check_ct : Dba.Expr.t -> is_cf:bool -> t -> Ct_status.t * t

  val check_ct_loads : Dba.Expr.t -> t -> Ct_status.t * t
  ...
  val toml_stats : unit -> Toml.Types.table
end
...
module Path_state (S : STATE) = struct
  type t = {
    ...
    address_counters : Checkct_options.Address_counter.t Virtual_address.Map.t;
  }
  ...
  let is_depth_ok ps =
    let max_depth = Checkct_options.MaxDepth.get () in
    ps.depth < max_depth

...
module AvoidAddresses = Builder.String_set (struct
  ...
  let doc = "set addresses where exploration should stop"
end)

module GoalAddresses = Builder.String_set (struct
  let name = "explore"

  let doc = "set addresses where exploration should try to go"
end)
...
module LoadROSections = Builder.False (struct
  ...
  let doc =
    "Load the content of all read-only sections (see also -checkct-load-sections)"
end)
...
module Directives = Builder.Any (struct
  ...
  let doc = "Set exploration directives"
...
module Taint = Builder.False (struct
  let name = "taint"

  let doc = "use taint to find instruction that can not leak"
end)

module ChosenValues = Builder.False (struct
  let name = "cv"

  let doc = "try to leak info using chosen secret values"
end)

module Relse = Builder.False (struct
  let name = "relse"

  let doc = "find insecurities using relational symbolic execution"
end)

module RelseIsDumb = Builder.False (struct
  let name = "relse-is-dumb"

  let doc = "disables RelSE internal taint mechanism (for testing purpose only)"
end)

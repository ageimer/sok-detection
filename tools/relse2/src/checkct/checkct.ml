...
module type CHECKCT_RUNNER = sig
  val start : unit -> unit
end

module Env_make (S : STATE) (WF : WORKLIST_FACTORY) : CHECKCT_RUNNER = struct
  let timestamp = Unix.gettimeofday ()

  module Explo_stats = struct
    type t = {
      paths : int;
      completed_paths : int;
      unknown_paths : int;
    ...
    let has_errors s = s.asserts_failed > 0 || s.unknown_paths > 0
    ...
    let toml s =
      ...
      Min.of_key_values
        [
          (Min.key "total paths", Types.TInt s.paths);
          (Min.key "competed/cut paths", Types.TInt s.completed_paths);
          ( Min.key "pending paths",
            Types.TInt (s.paths - s.completed_paths - s.unknown_paths) );
          (Min.key "stale paths", Types.TInt s.unknown_paths);
          (Min.key "failed assertions", Types.TInt s.asserts_failed);
          (Min.key "branching points", Types.TInt s.branches);
          (Min.key "max path depth", Types.TInt s.max_depth);
          (Min.key "visited instructions (unrolled)", Types.TInt s.instructions);
        ]
    ...
    module R = struct
      ...
      let has_errors () = has_errors !value
  ...
  module Env = struct
    ...
    type t = {
      ...
      addr_ct_status : Ct_status.t Virtual_address.Htbl.t;
    ...
    let create entrypoint =
      ...
      in
      {
        ...
        addr_ct_status = Virtual_address.Htbl.create 7;
        complete = false;
      }
    ...
    let update_from_cli e =
      ...
      Virtual_address.Set.iter
        ...
        (Checkct_utils.get_goal_addresses ());
      Virtual_address.Set.iter
        (fun v -> add_cut e v Dba.Expr.one)
        (Checkct_utils.get_avoid_addresses ())

    let choose e =
      let rec pick_one w =
        match W.pop w with
        | path_state, worklist when Path_state.may_lead_to_goal path_state ->
            Logger.debug ~level:3 "Selecting path #%d (among %d)"
    ...
    let add_addr_status e a status =
      Virtual_address.Htbl.replace e.addr_ct_status a status

    let is_addr_insecure e a =
      match Virtual_address.Htbl.find e.addr_ct_status a with
      | Ct_status.CT_Insecure _ -> true
      | (exception Not_found) | _ -> false

    let addr_status_report e =
      let l_insecure = ref [] in
      let l_unknown = ref [] in
      Virtual_address.Htbl.iter
        (fun vaddr status ->
          match status with
          | Ct_status.CT_Insecure _ -> l_insecure := vaddr :: !l_insecure
          | CT_Unknown -> l_unknown := vaddr :: !l_unknown
          | _ -> assert false)
        e.addr_ct_status;
      let l_insecure = List.sort_uniq Virtual_address.compare !l_insecure in
      let l_unknown = List.sort_uniq Virtual_address.compare !l_unknown in
      (l_insecure, l_unknown)
  end

  let toml_ct_report e =
    let open Toml in
    let l_insecure, l_unknown = Env.addr_status_report e in
    ...
    let insecurity_models =
      Min.of_key_values
        (Virtual_address.Htbl.fold
           (fun vaddr status l ->
             match status with
             | Ct_status.CT_Insecure model ->
                 let toml_model = Insec_model.toml model in
  ...
  let halt e =
    Logger.info
      "@[<v 0>%a@,\
       ...
       @,\
       @]"
      State.pp_stats () Explo_stats.pp () (C.nb_vertex e.Env.rocfg);
    let l_insecure, l_unknown = Env.addr_status_report e in
    let c_insecure, c_unknown =
      (List.length l_insecure, List.length l_unknown)
    in
    if LeakInfo.get () = InstrLeak then
      Logger.result "%d instructions are insecure and %d have status unknown"
        c_insecure c_unknown;
    if Explo_stats.has_errors () then
      Logger.result
        "Exploration encountered some errors (see exploration statistics";
    let status =
      if c_insecure > 0 then
        Virtual_address.Htbl.find e.addr_ct_status (List.hd l_insecure)
        (* CT_Insecure *)
      else if e.complete = false || c_unknown > 0 || Explo_stats.has_errors ()
      then CT_Unknown
      else CT_Secure
    in
    if StatsFile.is_set () then (
      let toml_data =
        Toml.Min.of_key_values
          [
            ( Toml.Min.key "State statistics",
              Toml.Types.TTable (State.toml_stats ()) );
            (Toml.Min.key "CT report", Toml.Types.TTable (toml_ct_report e));
            (Toml.Min.key "Exploration", Toml.Types.TTable (Explo_stats.toml ()));
          ]
      in
      let filename = StatsFile.get () in
      let oc = open_out_bin filename in
      let ppf = Format.formatter_of_out_channel oc in
      Toml.Printer.table ppf toml_data;
      close_out oc);
    Logger.result "Program status is : %a (%0.3f)" Ct_status.pp status
      (Unix.gettimeofday () -. timestamp);
    exit (Ct_status.exit_code status)

  let check_ct expr ?(loads_only = false) ~is_cf e ps =
    let state = Path_state.symbolic_state ps in
    let vaddr = Path_state.virtual_address ps in
    assert (not (loads_only && is_cf));
    if LeakInfo.get () = InstrLeak && Env.is_addr_insecure e vaddr then ps
      (* only one report per instruction *)
    else
      let res, state =
        if loads_only then State.check_ct_loads expr state
        else
          State.ctstatus_and
            (State.check_ct_loads expr)
            (State.check_ct expr ~is_cf)
            state
      in
      (match res with
      | CT_Insecure _ -> (
          Logger.debug ~level:5 "Expression %a is insecure"
            Dba_printer.Ascii.pp_expr expr;
          Env.add_addr_status e vaddr res;
          Logger.result "Instruction %a is insecure (%.3fs)" Virtual_address.pp
            vaddr
            (Unix.gettimeofday () -. timestamp);
          match LeakInfo.get () with HaltLeak -> raise Halt | _ -> ())
      | CT_Secure ->
          Logger.debug ~level:5 "Expression %a is secure"
            Dba_printer.Ascii.pp_expr expr
      | CT_Unknown ->
          Logger.debug ~level:5 "Expression %a status is unknown"
            Dba_printer.Ascii.pp_expr expr;
          Env.add_addr_status e vaddr res);
      Path_state.set_symbolic_state state ps
  ...
  module Eval = struct
    ...
    (* lvalue <- e *)
    let assignment ~lvalue ~rvalue e ps =
      (* generate the logical constraint, add it to the path predicate,
       * update symbolic_state.
       *)
      let ps = check_ct rvalue ~loads_only:true ~is_cf:false e ps in
      let ps =
        match lvalue with
        | Dba.LValue.Store (_, _, addr) -> check_ct addr ~is_cf:false e ps
        | _ -> ps
      in
      ps
    ...
    let havoc ~lvalue ?(secret = false) ss =
      let default_name = if secret then "bs_secret" else "bs_unknown" in
      match lvalue with
      | Dba.LValue.Var { name; size; _ } -> State.fresh name size ~secret ss
      | Dba.LValue.Restrict (var, { lo; hi }) ->
          let size = hi - lo + 1 in
          let nondet = Dba.Expr.var default_name size in
          State.assign var.name
            (Dba_utils.Expr.complement nondet ~hi ~lo var)
            (State.fresh default_name size ~secret ss)
      | Dba.LValue.Store (bytesize, dir, addr) ->
          let bitsize = 8 * bytesize in
          let nondet = Dba.Expr.var default_name bitsize in
          State.write ~addr nondet dir
            (State.fresh default_name bitsize ~secret ss)

    let nondet ~lvalue e ps =
      (* generate the logical constraint, add it to the path predicate,
       * update symbolic_state.
       *)
      let ps =
    ...
    let ite ~condition ~jump_target ~local_target e ps =
      Explo_stats.add_branch ();
      let ps = check_ct condition ~is_cf:true e ps in
      let addr = Path_state.virtual_address ps in
      let state = Path_state.symbolic_state ps in
      match State.test condition state with
      | exception Unknown ->
          Explo_stats.add_unknown_path ();
          Env.pick_path e
      | True symbolic_state ->
          let consequent =
      ...
      | Both { t = symbolic_state; f = symbolic_state' } ->
          ...
          let alternative =
            Path_state.set_block_index local_target
              (Path_state.set_symbolic_state symbolic_state' ps)
          in
          Explo_stats.add_path ();
          Env.pick_alternative addr ?consequent ~alternative e

    let dynamic_jump ~jump_expr e ps =
      Explo_stats.add_branch ();
      let n = JumpEnumDepth.get () in
      let ps = check_ct jump_expr ~is_cf:true e ps in
      let state = Path_state.symbolic_state ps in
      match State.split_on jump_expr ~n state with
      | [] | (exception Unknown) ->
          Explo_stats.add_unknown_path ();
          Env.pick_path e
      | x :: bx ->
    ...
    let assertion cond idx e ps =
      let ps = check_ct cond ~loads_only:true ~is_cf:false e ps in
      let state = Path_state.symbolic_state ps in
      match State.test cond state with
      | exception Unknown ->
          Explo_stats.add_unknown_path ();
          Env.pick_path e
      | True symbolic_state ->
    ...
    let go e ps =
      ...
      | Dba.Instr.Assign (lvalue, rvalue, idx) ->
          let ps = assignment ~lvalue ~rvalue e ps in
          Path_state.set_block_index idx ps
      | Dba.Instr.Nondet (lvalue, _, idx) ->
          let ps = nondet ~lvalue e ps in
          Path_state.set_block_index idx ps
  ...
  let loop_until ~halt e ps =
    ...
    and handle_cut vaddr e ps =
      ...
      | cuts -> (
          ...
          | exception Unknown ->
              ...
              None
          | None ->
              Logger.debug "@[<h>Directive :: cut path %d %@ %a@]"
                (Path_state.id ps) Virtual_address.pp vaddr;
              Explo_stats.terminate_path ();
              None
          | Some symbolic_state ->
    ...
    and eval vaddr e ps =
      ...
      Explo_stats.add_instruction ();
      Explo_stats.update_depth (Path_state.depth ps);
      loop_aux (Eval.go e ps)
    in
    try
      Sys.catch_break true;
      let timeout = Timeout.get () in
      if Float.is_finite timeout then (
  ...
  let initialize_state (e : Env.t) state =
    ...
    let state =
      ...
      | files ->
          ...
          let module P = Checkct_parser.Make (M) in
          List.fold_left
            (fun state filename ->
              if not (Sys.file_exists filename) then
                Logger.fatal "Cannot find sse configuration file %s" filename;
              let script =
                ...
                let parser = P.script and lexer = Checkct_lexer.token in
                Parse_utils.read_file ~parser ~lexer ~filename
              in
              List.fold_left
                (fun state -> function
                        ...
                        (fun ss name ->
                          try
                            ...
                            State.memcpy ~addr size
                              ...
                              ss
                          with Not_found ->
                            Logger.warning "Unable to find section %s" name;
                            ss)
                        state names
                  | Script.Pragma Reach_all ->
                      e.Env.reach_all <- true;
                      state
                  | Script.Secret lvalue ->
                      Logger.debug ~level:3 "Registering secret : %a"
                        Dba_printer.Ascii.pp_lhs lvalue;
                      Eval.havoc ~lvalue ~secret:true state)
                state script)
            state files
    in
    state

  let do_checkct ~filename =
    let level = 3 in
    Logger.debug ~level "Running CheckCT on %s" filename;
    ...
    Logger.debug ~level "@[<h>Initializing CheckCT goals ...@]";
...
let _ = Cli.Boot.enlist ~name:"CheckCT" ~f:run

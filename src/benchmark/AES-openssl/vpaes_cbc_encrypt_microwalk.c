...
extern void RunTarget(uint8_t* input)
{
    ...
    vpaes_set_encrypt_key(input, KEYLEN*8, &enc_key);
    ...
    vpaes_set_decrypt_key(input, KEYLEN*8, &dec_key);
    vpaes_cbc_encrypt(ciphertext, dec_out, (size_t) DATALEN, &dec_key, iv_dec, 0);      

...
uint8_t do_one_computation(uint8_t *data) {
    ...
    size_t outl;
    mbedtls_rsa_rsaes_oaep_decrypt(&rsa_ctx, mbedtls_ctr_drbg_random, &rng_ctx, NULL, 0,  &outl, data+=ELEN, dec_out, NLEN);
...
void prepare_inputs(dudect_config_t *c, uint8_t *input_data, uint8_t *classes) {
    ...
    for (size_t i = 0; i < c->number_measurements; i++) {
        ...
        if (classes[i] == 0) {
            ...
            mbedtls_rsa_rsaes_oaep_encrypt(&rsa, mbedtls_ctr_drbg_random, &rng_ctx, NULL, 0,  DATALEN, plaintext, idx+=ELEN);

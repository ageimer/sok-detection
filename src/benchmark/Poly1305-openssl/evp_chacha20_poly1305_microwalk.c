...
extern void RunTarget(uint8_t* input)
{
    ...
    EVP_EncryptInit_ex(ctx, EVP_chacha20_poly1305(), NULL, input, iv);
    ...
    EVP_CIPHER_CTX_set_padding(ctx, 0);
    EVP_DecryptInit_ex(ctx, EVP_chacha20_poly1305(), NULL, input, iv);
    out_len = 0;
...
{
    // Initializing OpenSSL with only Chacha20-Poly1305 loaded
